...
export default function Card({ x, y, position, value, key, ended }: CardParams) {
  ...
  switch (value.suit) {
    case 'clubs':
      suit = <Club className="h-full w-2/3 mt-2 mb-1 mr-4" />
      break
    case 'spades':
      suit = <Spade className="h-full w-2/3 mt-2 mb-1 mr-4" />
      break
    case 'hearts':
      suit = <Heart className="h-full w-2/3  mt-2 pl-8 mb-1 mr-4" />
      break
    default:
      suit = <Diamond className="h-full w-2/3 mt-2 mb-2 mr-4" />
  }
  /**
  ...
  return (
    ...
    >
      {interpolatedStyle => (
        <div
          className="absolute w-12 h-18 sm:h-18 md:h-24 lg:h-44 xl:h-44 2xl:h-60 rounded-lg bg-white shadow-2xl  max-h-12 sm:max-h-14 md:max-h-14 lg:max-h-16 xl:max-h-18  2xl:max-h-30  z-30"
          style={{
